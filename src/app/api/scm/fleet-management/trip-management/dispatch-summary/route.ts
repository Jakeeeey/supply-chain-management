...
export async function GET(req: NextRequest) {
  try {
    ...
    const [
      ...
      fetchDirectus(`/items/post_dispatch_plan?limit=${encodeURIComponent(limit)}&fields=id,doc_no,driver_id,vehicle_id,starting_point,time_of_dispatch,time_of_arrival,estimated_time_of_dispatch,estimated_time_of_arrival,status,date_encoded`),
      fetchDirectus(`/items/post_dispatch_plan_staff?limit=${encodeURIComponent(limit)}&fields=post_dispatch_plan_id,user_id,role`),
      fetchDirectus(`/items/post_dispatch_invoices?limit=${encodeURIComponent(limit)}&fields=id,post_dispatch_plan_id,invoice_id,status`),

      // IMPORTANT: avoid forbidden "id" field on sales_invoice (use invoice_id)
      fetchDirectus(`/items/sales_invoice?limit=-1&fields=invoice_id,total_amount,customer_code,salesman_id`),

      fetchDirectus(`/items/vehicles?limit=-1&fields=vehicle_id,vehicle_plate`),
      fetchDirectus(`/items/user?limit=-1&fields=user_id,user_fname,user_lname`),
      fetchDirectus(`/items/customer?limit=-1&fields=customer_code,customer_name,city,province`),
      fetchDirectus(`/items/salesman?limit=-1&fields=id,salesman_name`),
    ]);

    // Handle upstream failures
    const all = [plansRes, staffRes, invRes, salesInvRes, vehiclesRes, usersRes, customersRes, salesmenRes];
    const failed = all.find((x) => !x.ok);
    if (failed && !failed.ok) {
      return NextResponse.json(
        { error: "Upstream request failed", details: failed.body, upstream_status: failed.status },
        { status: 500 }
      );
    }
    ...
    const userMap = new Map<string, string>(
      users.map((u: any) => [String(u.user_id), `${u.user_fname ?? ""} ${u.user_lname ?? ""}`.trim()])
    );

    const vehicleMap = new Map<string, string>(vehicles.map((v: any) => [String(v.vehicle_id), String(v.vehicle_plate ?? "")]));

    const salesmanMap = new Map<string, string>(salesmen.map((s: any) => [String(s.id), String(s.salesman_name ?? "")]));

    const salesInvoiceMap = new Map<string, any>(salesInvoices.map((si: any) => [String(si.invoice_id), si]));

    const customerMap = new Map<string, any>();
    customers.forEach((c: any) => {
      if (c.customer_code) customerMap.set(normalizeCode(String(c.customer_code)), c);
    });
    ...
    const mappedPlans = rawPlans.map((plan: any) => {
      ...
      if (representativeInvoice) {
        const si = salesInvoiceMap.get(String(representativeInvoice.invoice_id));
      ...
      const customerTransactions = planInvoices.map((inv: any) => {
        ...
        if (si) {
          ...
          if (si.customer_code) {
            const cObj = customerMap.get(normalizeCode(String(si.customer_code)));
            if (cObj) {
              customerName = String(cObj.customer_name ?? customerName);
              address = `${cObj.city || ""}${cObj.city ? ", " : ""}${cObj.province || ""}`.trim() || "N/A";
            }
          }
        }
    ...
    return NextResponse.json(
      { error: "Failed to load dispatch summary data", details: String(err?.message || err) },
      { status: 500 }
    );
  }
}
