...
export async function POST(req: NextRequest) {
    try {
        ...
        console.log(`[Category Upload API] Uploading file: ${file.name} (${file.size} bytes)`);

        const directusFormData = new FormData();
        directusFormData.append("file", file);
        directusFormData.append("title", `Category Image - ${new Date().toISOString()}`);
        ...
        if (!response.ok) {
            ...
            console.error(`[Category Upload API] Directus Error (${response.status}):`, errorText);
        ...
        console.log(`[Category Upload API] Upload successful. File ID: ${data.data.id}`);
