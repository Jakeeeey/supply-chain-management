                ...
                <div className="bg-background border-b border-border shrink-0">
                    ...
                    <div className="px-6 py-3 bg-slate-50 flex items-end gap-4">
                        ...
                        <div className="flex-1 space-y-1.5">
                            <label className="text-[10px] font-black uppercase text-slate-400 ml-1">Search Products</label>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                                {props.products.map((product) => {
                                    ...
                                    const selectedItem = props.tempCart.find((x) => x.id === product.id);
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="p-4 flex-1 flex flex-col">
                                                <div className="flex justify-between items-start gap-2 mb-2">
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="text-[10px] font-mono text-slate-400 mt-1 uppercase tracking-tighter">SKU: {product.sku}</p>
                                                    </div>
                                                    {selected && (
                                                ...
                                                <div className="mt-2 py-2 border-t border-slate-50">
                                                    <div className="flex items-baseline gap-1">
                                                        <span className="text-lg font-black text-primary">{money.format(product.price)}</span>
                                                        <span className="text-[10px] font-bold text-slate-400 uppercase">/ {product.uom}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="p-3 bg-slate-50/50 border-t border-slate-100 mt-auto space-y-2">
                                                <select
                                                    value={selectedItem?.selectedUom ?? product.uom}
                                                    onChange={(e) => props.onUpdateTempUom(product.id, e.target.value)}
                                                    className="w-full h-8 px-2 text-[11px] font-bold border border-slate-200 rounded bg-white outline-none focus:ring-1 focus:ring-primary"
                                                >
                                                    {(product.availableUoms || [product.uom]).map((uom) => (
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar bg-slate-50/30">
                            {selectedCount === 0 ? (
                                <div className="flex flex-col items-center justify-center h-full text-center px-6">
                                    ...
                                    <p className="text-xs font-bold text-slate-400 uppercase tracking-tight">Empty Cart</p>
                                    <p className="text-[10px] text-slate-400 mt-1 max-w-[180px]">Select products from the grid to add them to your order.</p>
                                </div>
                            ) : (
                                props.tempCart.map((item) => (
                                    <div key={item.id} className="group relative bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow-md transition-all overflow-hidden">
                                        <button
                                        ...
                                        <div className="p-3 pt-4">
                                            ...
                                            <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-50">
                                                ...
                                                <div className="text-right">
                                                    <p className="text-[8px] uppercase font-black text-slate-400 leading-none mb-0.5">Subtotal</p>
                        ...
                        <div className="p-5 border-t border-border bg-white shrink-0 space-y-4 shadow-[0_-4px_20px_rgba(0,0,0,0.04)]">
                            <div className="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-100">
                                ...
                                <span className="text-xl font-black text-primary tracking-tight">
                                    {money.format(props.tempCart.reduce((sum, item) => sum + (item.price * item.orderQty), 0))}
                                </span>
                            </div>
