...
import { generateStockTransferPicklistPDF } from '../../utils/generate-stock-transfer-pdf';

interface StockTransferPicklistPreviewProps {
  open: boolean;
  onClose: () => void;
  orderNo: string;
  pickerName: string;
      ...
      >
        <DialogHeader className="px-6 py-4 border-b border-border bg-muted/20 shrink-0">
          <DialogTitle className="text-sm font-bold tracking-tight text-foreground flex items-center gap-2">
            ...
            Stock Transfer Picklist Preview
        ...
        <div className="flex-1 bg-muted/50 overflow-hidden relative">
          {generating || !pdfUrl ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
              ...
              <span className="text-[10px] uppercase font-black tracking-widest text-muted-foreground animate-pulse">Building Picklist...</span>
            </div>
          ) : (
            <iframe
              key={pdfUrl}
