 ...
 * as an alias, we must fetch it directly from the junction table.
 */
export async function fetchProductSuppliers(productIds: number[]): Promise<Record<number, any[]>> {
  if (productIds.length === 0) return {};
  ...
  for (let i = 0; i < uniqueIds.length; i += CHUNK_SIZE) {
    ...
    const res = await fetchItems<any>("items/product_per_supplier", params);
    allRecords.push(...(res.data || []));
  }
