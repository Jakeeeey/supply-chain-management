...
export function useStockTransferApproval() {
  ...
  const updateStatus = async (orderNo: string, status: 'approved' | 'rejected') => {
    const group = base.baseOrderGroups.find((g) => g.orderNo === orderNo);
    if (!group) return;
