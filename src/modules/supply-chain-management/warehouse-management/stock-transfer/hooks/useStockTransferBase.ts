...
import type { StockTransferRow, BranchRow, OrderGroup } from '../types/stock-transfer.types';
...
export function useStockTransferBase({ statuses, autoFetch = true }: UseStockTransferBaseProps) {
  ...
  const fetchTransfers = useCallback(async () => {
    ...
    setFetchError(null);
    try {
      const res = await stockTransferLifecycleService.fetchTransfers(statuses.join(','));
