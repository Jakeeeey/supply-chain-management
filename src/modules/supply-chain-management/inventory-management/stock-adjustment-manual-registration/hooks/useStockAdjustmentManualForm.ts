...
export function useStockAdjustmentManualForm() {
  ...
  const fetchBranches = useCallback(async () => {
    try {
      const response = await fetch("/api/scm/inventory-management/stock-adjustment-manual-registration/branches");
      if (!response.ok) {
  ...
  const fetchProducts = useCallback(async (search?: string) => {
    ...
    try {
      const response = await fetch(
        `/api/scm/inventory-management/stock-adjustment-manual-registration/products${search ? `?search=${search}` : ""}`
      );
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/scm/inventory-management/stock-adjustment-manual-registration/suppliers");
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
      const result = await response.json();
  ...
  const fetchBranchInventory = useCallback(async (branchId: number) => {
    ...
    try {
      const response = await fetch(
        `/api/scm/inventory-management/stock-adjustment-manual-registration/branch-inventory?branchId=${branchId}`
      );
  ...
  const fetchInventory = useCallback(async (productId: number, branchId: number): Promise<number> => {
    try {
      const response = await fetch(
        `/api/scm/inventory-management/stock-adjustment-manual-registration/inventory?productId=${productId}&branchId=${branchId}`
      );
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
  ...
  const fetchNextDocNo = useCallback(async (type: "IN" | "OUT"): Promise<string> => {
    try {
      const response = await fetch(`/api/scm/inventory-management/stock-adjustment-manual-registration/next-doc-no?type=${type}`);
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
      const result = await response.json();
