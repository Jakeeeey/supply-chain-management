...
export function useStockAdjustmentManualForm() {
  ...
  const fetchBranches = useCallback(async () => {
    try {
      const response = await fetch("/api/scm/inventory-management/stock-adjustment-manual-posting/branches");
      if (!response.ok) {
  ...
  const fetchProducts = useCallback(async (search?: string) => {
    ...
    try {
      const response = await fetch(
        `/api/scm/inventory-management/stock-adjustment-manual-posting/products${search ? `?search=${search}` : ""}`
      );
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/scm/inventory-management/stock-adjustment-manual-posting/suppliers");
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
      const result = await response.json();
  ...
  const fetchProductsBySupplier = useCallback(async (supplierId: number, search?: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/scm/inventory-management/stock-adjustment-manual-posting/products?${params.toString()}`
      );
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
  ...
  const fetchBranchInventory = useCallback(async (branchId: number) => {
    ...
    try {
      const response = await fetch(
        `/api/scm/inventory-management/stock-adjustment-manual-posting/branch-inventory?branchId=${branchId}`
      );
  ...
  const fetchById = useCallback(async (id: number): Promise<StockAdjustmentManualDetail> => {
    const response = await fetch(`/api/scm/inventory-management/stock-adjustment-manual-posting/${id}`);
    if (!response.ok) {
      throw new Error(`HTTP ${response.status}: ${response.statusText}`);
    }
    const result = await response.json();
  ...
  const fetchInventory = useCallback(async (productId: number, branchId: number): Promise<number> => {
    try {
      const response = await fetch(
        `/api/scm/inventory-management/stock-adjustment-manual-posting/inventory?productId=${productId}&branchId=${branchId}`
      );
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
  ...
  const fetchNextDocNo = useCallback(async (type: "IN" | "OUT"): Promise<string> => {
    try {
      const response = await fetch(`/api/scm/inventory-management/stock-adjustment-manual-posting/next-doc-no?type=${type}`);
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      }
      const result = await response.json();
  ...
  const createAdjustment = useCallback(async (values: StockAdjustmentManualFormValues) => {
    const response = await fetch("/api/scm/inventory-management/stock-adjustment-manual-registration", {
      method: "POST",
  ...
  const postAdjustment = useCallback(async (id: number) => {
    const response = await fetch(`/api/scm/inventory-management/stock-adjustment-manual-posting/${id}/post`, {
      method: "POST",
    });
    const result = await response.json();
    if (result.error) throw new Error(result.error);
