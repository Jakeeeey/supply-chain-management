 ...
 */
export function useStockAdjustmentManual(defaultStatus?: string) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/scm/inventory-management/stock-adjustment-manual-posting?${queryParams.toString()}`
      );
  ...
  return {
    ...
    unfilteredData: filteredData, // Expose the whole filtered list
    totalItems,
    currentPage,
    setCurrentPage,
    totalPages,
