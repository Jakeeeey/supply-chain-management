...
export async function getRawRfidsByItemIds(itemIds: number[]) {
  ...
  return directusGet<{ data: Record<string, unknown>[] }>(`/items/rts_item_rfid?${params}`);
}
...
export async function getExistingRelatedIds(id: string) {
  const itemsJson = await directusGet<{ data: Record<string, unknown>[] }>(`/items/rts_items?filter[rts_id][_eq]=${id}&fields=id`);
  const itemIds = (itemsJson.data || []).map(i => i.id as number);
  
  let rfidIds: number[] = [];
  if (itemIds.length > 0) {
    const rfidJson = await directusGet<{ data: Record<string, unknown>[] }>(
        `/items/rts_item_rfid?filter[rts_item_id][_in]=${itemIds.join(",")}&fields=id`
    );
    rfidIds = (rfidJson.data || []).map(r => r.id as number);
  }
