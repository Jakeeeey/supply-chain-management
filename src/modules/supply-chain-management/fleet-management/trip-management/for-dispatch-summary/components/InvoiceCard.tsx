...
import type { GroupedDispatchInvoice } from "../types/for-dispatch-summary.types";
import {
  formatCurrency,
  buildAddress,
        ...
        <AccordionItem value="item-1" className="border-b-0">
          ...
          <AccordionTrigger className="px-4 py-3 hover:no-underline [&[data-state=open]>div>svg]:rotate-180">
            <div className="flex w-full flex-col gap-2 text-left">
              {/* Customer Name */}
              <div className="mt-1">
                <p className="text-xs font-semibold text-foreground break-words whitespace-normal leading-tight">
                  {group.customerName}
                </p>
              </div>
          ...
          <AccordionContent className="px-4 pb-3 space-y-3">
            ...
            {/* Invoice (Replacing Order ID as requested) */}
            {firstInvoice.orderId && isSingleInvoice && (
              <div className="space-y-0.5">
                <p className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                  Invoice
                </p>
                <p className="text-xs text-foreground">{firstInvoice.orderId}</p>
              </div>
            )}
                  ...
                  {isExpanded && (
                    <div className="mt-2 space-y-1.5 bg-muted/30 rounded-md p-2">
                      {group.invoices.map((invoice) => (
                        ...
                        >
                          <div className="flex items-center gap-2 min-w-0 flex-col items-start">
