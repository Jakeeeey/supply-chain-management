...
export default function SupplierReliabilityScorecard() {
    ...
    const avgFulfillment = items.length > 0
                ...
                <CardContent>
                    <DataTable
                        columns={[
                            ...
                            { accessorKey: 'avgLeadTime', header: 'Avg Lead Time (Days)', cell: ({ row }) => row.original.avgLeadTime.toFixed(1) },
                            {
                                    ...
                                    return (
                                        <div className="flex items-center gap-2">
                                            <div className="w-16 h-2 bg-muted rounded-full overflow-hidden">
                                                <div
                                                    className={cn("h-full", rate < 95 ? "bg-rose-500" : "bg-emerald-500")}
                                                    style={{ width: `${rate}%` }}
                                                />
                                            </div>
                                            <span className={cn(rate < 95 ? "text-rose-600 font-bold" : "text-emerald-600")}>
                                                {rate.toFixed(1)}%
                                            </span>
                                        </div>
                                    );
                            ...
                            },
                            {
                                ...
                                cell: ({ row }) => (
                                    row.original.isAtRisk ?
                                        ...
                                        <Badge variant="outline" className="text-emerald-500 border-emerald-200 text-xs">Preferred</Badge>
                                )
                            },
                        ]}
                        data={items}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="md:col-span-2">
                    ...
                    <CardContent className="h-[200px] flex items-center justify-center text-muted-foreground italic">
                        Lead time trend visualization pending historical snapshot integration...
                    </CardContent>
                </Card>
                <Card>
                    <CardHeader>
                        ...
                        <div className="space-y-4">
                            {atRiskSuppliers.length > 0 ? (
                                atRiskSuppliers.map((supplier) => (
                                    <div key={supplier.supplierId} className="flex items-start gap-3 p-3 rounded-lg bg-rose-50 border border-rose-100">
                                        <TrendingUp className="h-5 w-5 text-rose-500 mt-0.5" />
                                        <div>
                                            <p className="text-sm font-semibold text-rose-900">{supplier.supplierName}</p>
                                            <p className="text-xs text-rose-700">Fulfillment at {supplier.avgFulfillmentRate.toFixed(1)}% this period.</p>
                                        </div>
                                    </div>
                                ))
                            ) : (
                                <p className="text-sm text-muted-foreground">All suppliers are meeting fulfillment thresholds.</p>
                            )}
                        </div>
                    </CardContent>
                </Card>
