...
export default function InventoryPerformanceDashboard() {
  ...
  const {
    ...
    error,
    refresh,
  } = useInventoryPerformance(filters);
  ...
  if (error) {
    return (
      <div className="space-y-6">
        ...
        />
        <ErrorPage title="Data Fetch Error" message={error} reset={refresh} />
      </div>
    );
  }
  ...
  const totalValue = inventoryPerformance.items.reduce(
    (sum, i) => sum + (i.value ?? 0),
    0,
  );
  const totalSKUs = inventoryPerformance.items.length;
  const abcACount = inventoryPerformance.items.filter(
    (i) => i.abcValueClass === "A",
  ).length;
  const fastMovers = inventoryPerformance.fnsDistribution.find(
    (d) => d.label === "Fast",
  );
          ...
          <DataTable
            columns={[
              ...
              { accessorKey: "name", header: "Product Name" },
              {
                accessorKey: "value",
                header: "Value (₱)",
                cell: ({ row }) => (row.original.value ?? 0).toLocaleString(),
              },
