...
export const SalesOrderModal: React.FC<SalesOrderModalProps> = ({ order, open, onClose, onUpdateRemarks }) => {
    ...
    const handleSaveRemarks = async () => {
        setIsSaving(true);
        try {
            await InvoicingService.updateSalesOrderRemarks(order.order_id, remarks);
            onUpdateRemarks(order.order_id, remarks);
                                    ...
                                    >
                                        ...
                                        <Card className="bg-muted/10 border-primary/10">
                                            ...
                                            <CardContent className="p-3 md:p-6 pt-0 md:pt-0 flex flex-col items-center text-center justify-center">
                                                <p className="font-bold text-xs md:text-sm break-words max-w-[200px]">{order.supplier_id?.supplier_shortcut || "—"}</p>
                                        ...
                                        <div className="md:col-span-2 space-y-2 mt-4">
                                            ...
                                            <div className="relative group">
                                                <Textarea
                                                    ...
                                                    className="min-h-[80px] md:min-h-[100px] bg-muted/5 border-primary/10 focus-visible:ring-primary/40 focus:bg-background transition-all duration-300 rounded-xl text-xs"
                                                />
                                                <div className="absolute bottom-2 right-2">
                                                    <Button 
                                                        ...
                                                        disabled={isSaving || order.remarks === remarks}
                                                        className="h-7 md:h-8 rounded-lg hover:bg-primary hover:text-primary-foreground font-bold text-[10px] md:text-xs"
                                                    >
                                                        {isSaving ? <Loader2 className="h-3 w-3 mr-1 md:mr-2 animate-spin" /> : <Save className="h-3 w-3 mr-1 md:mr-2" />}
                                                        SAVE
                                                    </Button>
                                                </div>
                                            </div>
